...
# Convert catelogical variables into numerical ones
...
# Copy NeighborhoodBin into a temporary DataFrame
...
# Blend the results and save the prediction to a CSV file.
# the value of alphaa could be changed to get a best score in this competition
alphaa = 2
y_pred = (y_pred_xgb + y_pred_lasso) / alphaa
y_pred = np.exp(y_pred)

pred_df = pd.DataFrame(y_pred, index=test_df["Id"], columns=["SalePrice"])
pred_df.to_csv('final_result.csv', header=True, index_label='Id')
